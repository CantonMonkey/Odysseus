"""
eval_suite_v2.py — Different-object evaluation suite: 电视 / 冰箱 / 桌子.

Tests the same agent on goal objects completely different from the training
objects (沙发, 椅子, 床) used in eval_suite.py. Validates generalisation
across object categories and room types.

Object selection rationale
--------------------------
电视  (television)  : living_room  — 33 instances in scene 00800
冰箱  (refrigerator): kitchen      — 23 instances
桌子  (table)       : any room     — 82 instances (most common furniture)

These span three distinct semantic rooms and require different navigation
strategies from the baseline suite.

Usage
-----
python eval_suite_v2.py [--episodes N]   # default 5 episodes per goal
"""
...
SUITES = [
    ...
    ("easy_v2_single",  ["电视", "桌子"],          False, "Single-goal: living-room objects (电视, 桌子)"),
    ("mid_v2_single",   ["电视", "冰箱", "桌子"],  False, "Single-goal: multi-room objects (电视, 冰箱, 桌子)"),
    ("hard_v2_chain",   ["电视", "冰箱", "桌子"],  True,  "Chain: 电视→冰箱→桌子, shared map"),
]

SUCCESS_DIST = 3.0


# ── Helpers (identical to eval_suite.py) ───────────────────────────────────────
    ...
    for label, goals, chain, description in SUITES:
        ...
        try:
            ...
            summary_lines.append(
                f"{label:<20} SR={overall['sr']*100:.1f}%  SPL={overall['spl']:.3f}"
            ...
            summary_lines.append(f"{label:<20} ERROR: {e}")

    total_min = (time.time() - t0_total) / 60
    print(f"\n{'='*68}")
    print(f"  EVAL SUITE V2 COMPLETE  ({total_min:.0f} min total)")
    print(f"{'='*68}")
    for line in summary_lines:
        print(f"  {line}")
    print(f"{'='*68}\n")

    out_json = Path("/tmp/suite_v2_results.json")
    out_txt  = Path("/tmp/suite_v2_summary.txt")
